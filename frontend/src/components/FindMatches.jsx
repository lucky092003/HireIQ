...
export default function FindMatches({
  ...
  }, [loading, jobDescription, jobTitle, jobLocation, keySkills, experienceYears]);

  // recent searches removed per request
  ...
  const stats = useMemo(() => {
    if (!results.length) {
      return { averageScore: 0, totalSkills: 0, topScore: 0 };
    }

    const totalScore = results.reduce((sum, candidate) => sum + candidate.match_score, 0);
    const totalSkills = results.reduce((sum, candidate) => sum + (candidate.matched_skills?.length || 0), 0);
    const topScore = Math.max(...results.map((candidate) => candidate.match_score));
  ...
  const findMatches = async () => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:8000/api/hireiq/match", {
        method: "POST",
        headers: {
      ...
      // only toggle internal results panel when enabled
  ...
  return (
    <div className="find-matches-container">
      <button onClick={findMatches} disabled={loading} className="find-matches-btn">
        {loading ? (
          <>
      ...
      <div className="search-hint">Tip: Press Ctrl + Enter to search</div>
